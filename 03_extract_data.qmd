...
- `start_XXX`/`end_XXX` - Start/End times of the *first* and *last* visit/presence bout of the day (`visit`/`presence`).
- `length` - Duration in **seconds** of the visit/presence bout
- `time2next` - Time in **minutes** to the next visit/presence bout
- `visits_n`/`bouts_n` - Daily number of visits/presence bouts
- `animal_n` - Number of animals in the data
- `logger_n` - Number of loggers in the data
- `min_XXX`/`max_XXX`/`median_XXX`/`mean_XXX` - Daily Min/Max/Median/Mean of the measure in question (`len` = `length`, `t2next` = `time2next`)
